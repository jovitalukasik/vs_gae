...
class NodeEmb(nn.Module):
    def __init__(self, ndim, num_layers, node_dropout, dropout):
        ...
        self.NodeInit = nn.Embedding(5, ndim)
...
class GetAcc_fix(nn.Module):
    def __init__(self, sdim, num_layers, dropout):
        super(GetAcc_fix, self).__init__()
        self.sdim = sdim
        self.num_layers = num_layers
        self.dropout = dropout
        ...
        in_channels = sdim
        for out_channels in np.linspace(1, sdim, num=num_layers)[-2::-1]:
            out_channels = int(np.round(out_channels))
            self.lin_layers.append(nn.Linear(in_channels, out_channels))
